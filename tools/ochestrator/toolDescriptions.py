class ToolPrompts:
    ...
    - columns: list[{name: string, type: string}] | null
    - user_confirmation: bool | null

    RULES:
    You can skip a single or multiple steps if you already have the required information. For example, if you already have the table_name, you can skip the ask_table_name step. However, you cannot skip a step or steps if the required information is missing.
    ...
    - table_type: string | null
    RULES:
    You can skip a single or multiple steps if you already have the required information. For example, if you already have the table_type, you can skip the ask_table_type step. However,
    you cannot skip a step or steps if the required information is missing.
    ...
    You can skip a single or multiple steps if you already have the required information. For example, if you already have the table_name, you can skip the ask_table_name step. However, you cannot skip a step or steps if the required information is missing.
    1) ask user for table_name if not provided.
    2) fetch column names for the given table_name and ask user to select the column name and provide its corresponding value to identify the record(s) to be deleted. Store the selected column name and value in column_name and column_value respectively.
    3) fetch the record(s) based on the provided column_name and column_value. If no record is found, go back to step 2. If multiple records are found, ask user to provide the record_id of the record to be deleted and store it in record_id.
    4) ask for yes/no. Set user_confirmation accordingly.
    5) call delete_record ONLY if table_name, column_name, column_value are non-null AND user_confirmation=true. If record_id is available, use it to delete the specific record. Otherwise, delete all records matching the column_name and column_value.
    If any required field is missing, move back to the appropriate ask_* step.
    '''
